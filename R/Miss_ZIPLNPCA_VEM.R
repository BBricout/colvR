...
Miss.ZIPLNPCA_VEM <- function(Y, # Matrice de comptage 
                              X, # Covariables
 ...
 if(is.null(config_vem)){
   config_vem <- list(maxiter = 100000, ftol = 1e-08, xtol = 1e-06, 
  ...
  if(is.null(params)){
    # params_q0 <- Init_ZIP_q0(Y, X, q)
    # out <- nlopt_optimize_ZIP_q0(data, params, config, tolxi)
    # params <- list(B = out$B, D = out$B)
  ...
  while((max(abs(params_new - params_old)) > xtol && abs(elbo_new - elbo_old) > ftol) && iter < maxiter){
    ...
    params <- list(B = outVE$B, D = outVE$D, C = outVE$C,
                   M = outVE$M, S = outVE$S)
